...
fn remove_dir_loop(path: &Path, dir_name: &str, amount_of_existing_files: usize) {
    let another_path = format!("{}/{}", path.as_os_str().to_str().unwrap(), dir_name);

    fs::remove_dir_all(&another_path).unwrap();
